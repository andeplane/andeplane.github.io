...
import { Dialog, DialogContent, DialogTitle, DialogDescription, DialogTrigger } from '@/features/neural-operators/labs/components/ui/dialog';
...
export function NeuralKernelLab() {
  ...
  return <>
    <div className="paper-actions"><Dialog><DialogTrigger className="paper-button">⚙ Preferences</DialogTrigger><DialogContent style={{maxWidth:620,maxHeight:'85vh',overflowY:'auto',backgroundColor:'#fff',color:'#172338',boxShadow:'0 24px 80px rgba(15,30,50,.22)'}}><DialogTitle>Kernel experiment preferences</DialogTitle><DialogDescription>Changes reset training. Layers are hidden layers inside the kernel network, not time steps or stacked operator layers.</DialogDescription>
      ...
      <Control label="Training field pairs" value={trainingOptions.count} min={1} max={64} onChange={count=>configureData({count})}/>
    ...
    </DialogContent></Dialog><span>{targetMode==='nonlinear'?`Temperature at Δt = ${horizon}`:'Linear smoothing'} · {architecture} · {aggregation} · layers {widths.join(' → ')} · {theta.length} parameters{withSource?' · source at 0.4':''}</span></div>
