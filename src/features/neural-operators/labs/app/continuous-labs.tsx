...
export function NeuralKernelLab() {
  ...
  return <>
    <div className="paper-actions"><Dialog><DialogTrigger className="paper-button">⚙ Preferences</DialogTrigger><DialogContent className="kernel-preferences"><div className="kernel-preferences-header"><DialogTitle>Experiment settings</DialogTitle><DialogDescription>Configure the training data and learned kernel. Changes to data or architecture reset training.</DialogDescription></div><div className="kernel-preferences-body"><section className="kernel-preferences-section"><h3>Compute</h3>
      ...
      <label htmlFor="source-mode">Output-label offset</label><NativeSelect id="source-mode" value={withSource?'yes':'no'} onChange={e=>configure(architecture,widths,targetMode,e.target.value==='yes')}><NativeSelectOption value="no">No added offset</NativeSelectOption><NativeSelectOption value="yes">Add a bump at x = 0.4</NativeSelectOption></NativeSelect><p>Adds a fixed bump to the target output after generating the field pair. It is not a heat source integrated through physical time. The model must fit this offset through its learned integrand.</p>
